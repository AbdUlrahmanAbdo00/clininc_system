...
@section('title', 'تعديل الشيفت - مركز صحي')
@section('page-title', 'تعديل الشيفت')
@section('page-description', 'تعديل بيانات الشيفت')

@section('content')
{{-- 
    المتغيرات المطلوبة من Controller:
    $shift = بيانات الشيفت المراد تعديله مع علاقته (doctor)
    $doctors = قائمة جميع الأطباء للاختيار
--}}
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow p-6">
        <div class="mb-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-2">تعديل معلومات الشيفت</h2>
            <p class="text-gray-600">تعديل بيانات الشيفت للطبيب: {{ $shift->doctor->name }}</p>
        </div>
        
        <form action="/dashboard/shifts/{{ $shift->id }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="day" id="day" 
                        ...
                        <option value="الأحد" {{ old('day', $shift->day) == 'الأحد' ? 'selected' : '' }}>الأحد</option>
                        <option value="الاثنين" {{ old('day', $shift->day) == 'الاثنين' ? 'selected' : '' }}>الاثنين</option>
                        <option value="الثلاثاء" {{ old('day', $shift->day) == 'الثلاثاء' ? 'selected' : '' }}>الثلاثاء</option>
                        <option value="الأربعاء" {{ old('day', $shift->day) == 'الأربعاء' ? 'selected' : '' }}>الأربعاء</option>
                        <option value="الخميس" {{ old('day', $shift->day) == 'الخميس' ? 'selected' : '' }}>الخميس</option>
                        <option value="الجمعة" {{ old('day', $shift->day) == 'الجمعة' ? 'selected' : '' }}>الجمعة</option>
                        <option value="السبت" {{ old('day', $shift->day) == 'السبت' ? 'selected' : '' }}>السبت</option>
                    </select>
                    @error('day')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
